...
function validate(grid, points) {
    ...
    for (let index = 0; index < points.length; index++) {
        ...
        if ((0 <= row && row < height && 0 <= col && col < width)) pRe.push(points[index]);
    }
    return pRe;
}
...
function getNeighbors(grid, visited, node) {
    ...
    neighbors.forEach(neighbor => {
        if (isVisited(visited, neighbor)) connected.push(neighbor);
        else unconnected.push(neighbor);
    });
...
function makeWall(grid, row, col, isW) {
    grid[row][col].isWall = isW;
}

export { dijkstra, BFS, DFS, getShortestPath, primMaze };